...
const useHandStore = create(
  persist(
    (set) => ({
      tiles: [],            // TileObject[] — closed hand tiles
      melds: [],            // { open: boolean, tiles: TileObject[] }[]
      analysisResult: null, // result from analysis.js — not persisted
    ...
    {
      name: 'riichi-hand',
      // Exclude derived/computed state from localStorage
      partialize: (state) => ({
        tiles: state.tiles,
        melds: state.melds,
        playerDiscards: state.playerDiscards,
      }),
    }
  )
)
